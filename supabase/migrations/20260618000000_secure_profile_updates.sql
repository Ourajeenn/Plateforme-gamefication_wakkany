...
--   Le client envoie un UPDATE/UPSERT direct avec la valeur d'XP calculée
--   côté navigateur : aucune validation serveur n'existait.
--
...
--   4. Utilisation de la variable de session `app.allow_profile_update`
--      pour autoriser temporairement la modification depuis les fonctions RPC sécurisées.
-- ============================================================================
...
RETURNS TRIGGER AS $$
BEGIN
  -- Permettre la modification par des fonctions autorisées via une variable de session
  ...
  IF NEW.xp IS DISTINCT FROM OLD.xp THEN
    RAISE EXCEPTION 'Modification directe du champ xp interdite, utilisez les fonctions sécurisées';
  END IF;
...
--    jamais imposer une valeur absolue arbitraire.
...
BEGIN
  ...
  -- Activer la variable de session pour autoriser le trigger
  ...
  -- Désactiver la variable de session
  PERFORM set_config('app.allow_profile_update', 'false', true);
...
--    et déduit le coût en XP du profil de l'utilisateur.
...
DROP POLICY IF EXISTS "Skill definitions are viewable by everyone" ON skill_definitions;
...
BEGIN
  ...
  -- Déduire l'XP de l'utilisateur
  PERFORM set_config('app.allow_profile_update', 'true', true);
  
  UPDATE profiles
  SET xp = xp - def.xp_cost,
      level = FLOOR((xp - def.xp_cost) / 100) + 1
  WHERE id = auth.uid();
