// Full App component moved from src/App.jsx
...
export default function App() {
  ...
  useEffect(() => {
    const questTimer = setInterval(() => {
      setFlashQuests((prev) =>
        prev
          .map((q) => ({ ...q, timeLeft: Math.max(0, q.timeLeft - 1) }))
          .filter((q) => q.timeLeft > 0)
      );
    }, 1000);
  ...
  useEffect(() => {
    ...
    ACHIEVEMENTS.forEach((achievement) => {
      if (
        ...
        achievement.condition({ user, xp, unlockedSkills, completedQuests })
      ) {
  ...
  return (
    <>
      ...
      >
        <iconify-icon icon="mdi:arrow-up" width="24" className="relative z-10" />
      </button>
