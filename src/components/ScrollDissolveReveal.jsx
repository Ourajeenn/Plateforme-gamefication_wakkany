...
const coverFragmentShader = `
  ...
  void main() {
    ...
    baseColor += edgeColor * edgeGlow * uEdgeBrightness;
    ...
    baseColor += vec3(sparkle * 3.0 * edgeBrightness);
...
function Scene({ imageFront, imageBack, scrollYProgress }) {
  ...
  useFrame((state) => {
    ...
    if (material2Ref.current) {
      ...
      material2Ref.current.uniforms.uEdgeIntensity.value = 0.6 * (1.0 - acc);
      material2Ref.current.uniforms.uDarkness.value = 1.0 - acc;
      material2Ref.current.uniforms.uGrayscale.value = 1.0 - acc;
    }
  });
